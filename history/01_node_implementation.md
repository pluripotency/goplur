...
また、`LogParams.EnableStdout` が有効な際に、実行結果だけでなく送られたコマンド（action）自体も標準出力およびログファイルに出力する機能を追加しました。この際、ターミナルのエコーバック（`stty echo`）を有効にすることなく、プログラム側で制御しています。
...
- **`session.go` ([session.go](file:///home/worker/Documents/antigravity/goplur/session.go))**:
  ...
  - `actionHandler()` メソッド内で、コマンド実行（action）時にそのコマンド文字列を `outputWriter` に直接書き出すようにし、標準出力およびログへ出力できるようにしました。
