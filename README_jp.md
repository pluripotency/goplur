...
> [!NOTE]
> **謝辞と感謝:**
> `goplur` における対話型セッション制御および期待値マッチング（Expect）のコアエンジンは、[google/goexpect](https://github.com/google/goexpect) ライブラリからコードを抽出・簡素化したものをベースに構築されています。素晴らしいライブラリを開発・公開してくださった `goexpect` の原著者に深く感謝いたします。抽出されたサブセットは本プロジェクト内（`src/expect/`）にインライン化されており、デフォルトの2秒のポーリング遅延を解消したミリ秒単位の超高速な動作、外部依存ライブラリ（gRPCやGoのSSHクライアント等）の完全な排除、ならびに標準の `goexpect` にはない対話型ターミナル引き渡し機能（`s.Interact()`）の追加を実現しています。
...
- **対話型ターミナルの引き渡し（`Interact`）**: 標準の `google/goexpect` には存在しない、Python の `pexpect.interact()` に相当する `s.Interact()` を提供。Tab 補完や矢印キー、シグナル処理に対応したフルターミナル操作をユーザーへ直接引き渡し可能。
...
### ユーザー対話への制御引き渡し（`Interact`）
標準の `google/goexpect`（および多くの Go 製 expect ライブラリ）はプログラムからの Expect/Send 自動化のみに対応しており、Python の `pexpect.interact()` のようにプロセスの制御をユーザーの端末に引き渡す（Session Handoff）機能は備わっていません。`goplur` ではこの機能を独自拡張し、`s.Interact()`（または `goplur.Interact(s)`）を提供しています。
- **Raw モードと端末エミュレーション**: ローカル端末を一時的に Raw モード（`term.MakeRaw`）に切り替えることで、行バッファリングを無効化し、Tab 補完、矢印キーによるカーソル移動、Ctrl+C、vim などの操作をリモートシェルへダイレクトに送信。
- **エコーバックの自動管理**: 対話開始時にリモート端末のエコーバック（`stty echo`）を自動有効化し、ログアウト時（`exit` や Ctrl+D）にローカル端末を安全に元のモード（Cooked モード）へ復元。
- **自動化と手動操作のハイブリッド**: 認証ログインや初期スクリプトの実行を自動化した後、`s.Interact()` を呼び出すだけで即座にユーザー操作へ引き渡すことができます。
